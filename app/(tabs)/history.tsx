...
import { useTranslationHistory } from '../../src/hooks/useTranslationHistory';
...
const modeIcon  = (mode: string) => mode === 'scan' ? '📷' : mode === 'paste' ? '📋' : '🌐';
const modeColor = (mode: string) => mode === 'scan' ? '#8B5CF6' : mode === 'paste' ? '#F59E0B' : colors.primary;
const modeLabel = (mode: string) => mode === 'scan' ? 'SCAN' : mode === 'paste' ? 'PASTE' : 'TRANSLATE';
  ...
  if (history.length === 0) {
    return (
      <View style={styles.empty}>
        ...
        <Text style={styles.emptySub}>Translations from Translate, Scan and Paste tabs will appear here.</Text>
      </View>
    );
  }
